...
class Optimizer:
    def __init__(self,k_d=0.0155,k_l=5.0269,k_w=5.0269):
        ...
        ydot1=y[4] * u[3]
        ydot2=y[5] * u[3]
        ydot3=(k_l*y[7]*(np.sin(lambd)*np.sin(gamma)*y[5]-np.cos(lambd)*y[4])-k_d*y[6]*y[3]) * u[3]
        ydot4=(k_l*y[7]*(np.sin(lambd)*np.cos(gamma)*y[5]-np.cos(lambd)*y[3])-k_d*y[6]*y[4]) * u[3]
        ydot5=(k_l*y[7]*(np.sin(lambd)*np.cos(gamma)*y[4]-np.sin(lambd)*np.sin(gamma)*y[3])-k_d*y[6]*y[5]-g) * u[3]
        ydot6=(k_l*y[7]*((np.sin(lambd)*np.sin(gamma)*y[5]-np.cos(lambd)*y[4])+(np.sin(lambd)*np.cos(gamma)*y[5]-np.cos(lambd)*y[3])+(np.sin(lambd)*np.cos(gamma)*y[4]-np.sin(lambd)*np.sin(gamma)*y[3]))/(y[6]) -k_d*(y[3]+y[4]+y[5])) * u[3]
        ydot7=(-k_w*y[7]*y[7]) * u[3]
    ...
    def find_initvalues_spin(self,target):
        ...
        sol_speed = sol['x'][0]
        sol_angle= sol['x'][1]*180/np.pi
        sol_spin = sol['x'][2]
        sol_tf = sol['x'][3]
        #print("Landed on: ", X_end)
        solution = [float(sol_speed),float(sol_angle),float(sol_spin),float(sol_tf)]
...
def Simulate_ballpath(self,y_0):
    ...
    r = integrate.ode(ballpathfunction).set_integrator("dopri5") 
...
def find_y0(self,x):
    ...
    omega_0=x[2]
    v_0=x[0]
    dx_0=x[0]*np.cos(x[1])*np.sin(theta)
    dy_0=x[0]*np.cos(x[1])*np.cos(theta)
    dz_0=x[0]*np.sin(x[1])
...
def plot_path(self,speed,angle,spin):
    x=[speed,angle,spin]
    y_0 = find_y0(x)
    [t_s,y_s]=Simulate_ballpath(y_0)
    path = aboveground(y_s[:,0:3])
    print("Path time: ", t_s)
    fig = plt.figure()
